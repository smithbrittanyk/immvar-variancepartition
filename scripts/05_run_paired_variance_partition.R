# Run variance partition analysis on paired ImmVar donors
...
# Restrict analysis to individuals observed in both cell types -----------

cell_types_per_individual <- tapply(
  metadata$cell_type,
  metadata$individual_id,
  function(x) length(unique(x))
)

paired_individuals <- names(
...
metadata <- metadata[
  metadata$individual_id %in% paired_individuals,
  ,
  drop = FALSE
]

paired_sample_ids <- metadata$geo_accession

expression <- expression[
  ,
  colnames(expression) %in% paired_sample_ids,
  drop = FALSE
]

message(
  "Paired individuals: ",
  length(paired_individuals)
)
...
message(
  ...
  paste(
    names(table(metadata$cell_type)),
    as.integer(table(metadata$cell_type)),
    sep = "=",
    collapse = ", "
  )
)
...
# Fit paired-donor variance-partition model -------------------------------
...
output_file <- file.path(
  results_dir,
  "immvar_paired_variance_partition.csv"
)
